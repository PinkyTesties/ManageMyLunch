...
useEffect(() => {
    if (cart.restaurant_id ) {
      axios.get(`http://localhost:8082/api/restaurants/${cart.restaurant_id}`)
      .then((res) => {
  ...
  const deleteCart = () => {
    axios.delete(`http://localhost:8082/api/cart/${cart._id}`)
      .then((res) => {
        console.log(res.data);
      })
      .catch(err => console.log(err))
...
function handleBuyNow() {
    ...
    axios.post('http://localhost:8082/api/completedCarts', completedCart)
  ...
  <h2>Date Created: {new Date(cart.date_created).toLocaleDateString()}</h2>
